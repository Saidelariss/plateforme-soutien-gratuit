...
export class PostService {
    ...
    validatePostByApprenti(postId:number,email:string):Observable<Boolean>{
        const validatePostByApprenti = { postId:postId,email:email};
        const headers = new HttpHeaders().set('Content-Type', 'application/json');
        return this.http.post<Boolean>("http://localhost:8080/api/apprenti/post/validate",validatePostByApprenti,{ headers: headers });
    }
}